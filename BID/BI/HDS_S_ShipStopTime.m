...
#import "HDS_S_ShipStopTime.h"
#import "UIView+AddLine.h"
#import "HDSGradientView.h"

#define COL0_WIDTH 134.0f
#define COL1_WIDTH 90.0f
#define COL2_WIDTH 60.0f
#define COL3_WIDTH 67.0f
#define COL4_WIDTH 54.0f
#define OTHER_COL_WIDTH 41.0f
#define COL_HEIGHT 20.0f
#define SMALL_COL0_WIDTH 110.0f
#define SMALL_COL1_WIDTH 75.0f
#define SMALL_COL2_WIDTH 60.0f
#define SMALL_COL3_WIDTH 60.0f
#define SMALL_COL4_WIDTH 50.0f
#define SMALL_OTHER_COL_WIDTH 35.0f
#define SMALL_COL_HEIGHT 15.0f

@implementation HDS_S_ShipStopTime{
...
- (void)viewDidLoad
{
    ...
    [self addPiePlotView:pieView1 toContainer:pieContainer1 title:@"船舶停时不同原因比例" useLegend:YES useLegendIcon:YES];
...
// 离港时间，船名，载重吨，停泊艘时，装卸，其他，港方原因，航方原因，货方原因，其他原因，自然因素停时
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        ...
        case 2: return @"wgt";
        case 3: return @"time";
        case 4: return @"workUnloadTim";
        case 5: return @"workOtherTim";
        case 6: return @"noworkPortTim";
        case 7: return @"noworkShipTim";
        case 8: return @"noworkCargoTim";
        case 9: return @"noworkOtherTim";
        case 10:return @"weatherTim";
        default:return @""; 
    }
}
...
- (UIView *)tableView:(HDSTableView *)_tableView multiRowHeaderInTableViewIndex:(NSInteger)tableViewIndex{
    ...
    CGRect rects[13] = {  
        CGRectMake(1, 0, col0Width, colHeight*2),// 离港时间
        CGRectMake(1+col0Width,0, col1Width, colHeight*2),//船名
        CGRectMake(1+col0Width+col1Width,0, col2Width, colHeight*2),//载重吨
        CGRectMake(1+col0Width+col1Width+col2Width,0, col3Width, colHeight*2),//停泊艘时
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width,0, col4Width+otherColWidth, colHeight),//生产性停时
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width,colHeight, col4Width, colHeight),//装卸
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width,colHeight, otherColWidth, colHeight),//其他
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width+otherColWidth,0, otherColWidth*4, colHeight),//非生产性停时
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width+otherColWidth*1,colHeight, otherColWidth, colHeight),//港方
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width+otherColWidth*2,colHeight, otherColWidth, colHeight),//航方
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width+otherColWidth*3,colHeight, otherColWidth, colHeight),//货方
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width+otherColWidth*4,colHeight, otherColWidth, colHeight),//其他
        CGRectMake(1+col0Width+col1Width+col2Width+col3Width+col4Width+otherColWidth*5,0, otherColWidth, colHeight*2)//自然因素
    };
    NSArray *titles = [NSArray arrayWithObjects:
                       @"离港时间",@"船名",@"载重吨",@"停泊艘时",@"生产停时",@"装卸",
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ShipStopTime_list" withExtension:@"json"]]];
    }else{
        NSString *url;
        url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SShipStopTime/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate, qEndDate, qCompany];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"船舶在港停时分析");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"船舶在港停时分析");
	} else{
        //        NSLog(@"Parser over.");
    }
}
