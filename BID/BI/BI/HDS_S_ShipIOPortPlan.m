...
@implementation HDS_S_ShipIOPortPlan{
...
//    ((CPTBarPlot *)[graph plotAtIndex:0]).lineStyle = [HDSUtil plotBorderStyle];
...
- (void)viewDidLoad
{
    ...
    headerNames = [NSArray arrayWithObjects:@"预抵时间",@"船名",@"货名",@"吨数",@"泊位", nil];

    tableView = [HDSUtil addTableViewInContainer:tableContainer smallView:isSmallView];
    tableView.dataSource = self;
    
//    CGRect frame = isSmallView?plotContainer.bounds:CGRectInset(plotContainer.bounds,1.0f,1.0f);
...
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    if(isSmallView){
        ...
        if(column == 1 || column == 2) return 80.0f;
        return 60.0f;
    }
    if(column == 0)
        return 157.0f;
...
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        case 0: return @"time";
        case 1: return @"ship_nam";
        case 2: return @"cargo_nam";
        case 3: return @"bill_wgt";
        case 4: return @"pre_berth_nam";
        default:return @"";
    }
}
...
- (IBAction)segmentChanged:(UISegmentedControl *)sender {
    if([sender selectedSegmentIndex] == 0){
        ...
        headerNames = [NSArray arrayWithObjects:@"预离时间",@"船名",@"货名",@"吨数",@"泊位", nil];
        qIoPort = @"listLeave";
    }
    [self loadData];
}
...
- (UIControl *)tableView:(HDSTableView *)_tableView accessoryViewForNode:(HDSTreeNode *)node atIndexPath:(NSIndexPath *)indexPath atTableIndex:(NSInteger) tableIndex{
    if(!isSmallView && tableIndex == 1){
        ...
        frame.origin = CGPointMake(270.0-size.width, (cellHeight-size.height)/2.0f);
        btn.frame = frame;
        return btn;
    }
    return nil;
...
-(void)constructPlot{
    ...
    [HDSUtil setTitle:@"近日到港航班艘次量和吨数" forGraph:graph withFontSize:isSmallView?14.0f:16.0f];
    [HDSUtil setInnerPaddingTop:6.0 right:65.0 bottom:30.0 left:45.0 forGraph:graph];
...
//	plotSpace.yRange	= [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(0.0) length:CPTDecimalFromFloat(50.0)];
    ...
    [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(7.99) length:CPTDecimalFromFloat(0.02)],
                                     nil];
...
//    newPlotSpace.delegate = [HDSUtil getPlotDelegate];
    ...
    [HDSUtil setAxis:y2 majorIntervalLength:0 minorTicksPerInterval:0 title:@"吨数" titleOffset:48.0 titleFontSize:isSmallView?12.0f:14.0f labelFontSize:isSmallView?10.0f:12.0f];
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ShipIOPortPlan_listArrive" withExtension:@"json"]]];
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ShipIOPortPlan_listArriveChart" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SShipPlan/%@.json?corps=%@",qIoPort,qCompany];
        ...
        // 图表数据
        NSString *url2 = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SShipPlan/%@Chart.json?corps=%@",qIoPort,qCompany];
...
-(void) getShipInfoByShipCod:(NSString *)shipCod corpCod:(NSString *)corpCod{
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ShipIOPortPlan_shipInfo" withExtension:@"json"]]];
    }else{
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SShipPlan/getShipData.json?shipCod=%@&corpCod=%@",shipCod,corpCod];
        NSURLRequest *theRequest=[NSURLRequest requestWithURL:[NSURL URLWithString:url] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:Http_Request_Timeout];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        for(int i=0; i<array.count; i++){
            ...
            NSNumber *y1 = [data objectForKey:@"wgt"];
