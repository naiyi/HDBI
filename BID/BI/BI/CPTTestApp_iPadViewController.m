...
{
	[super viewDidLoad];
    scatterPlotView = [[CPTGraphHostingView alloc] initWithFrame:CGRectMake(5.0f, 5.0f,290.0f,190.0f)];
    scatterPlotView.layer.masksToBounds = true;
    scatterPlotView.layer.cornerRadius = 7;
...
//    barChartView.frame	  = CGRectMake(0.0f, 0.0f, 340.0f, 340.0f);
//    pieChartView.frame	  = CGRectMake(0.0f, 644.0f, 340.0f, 340.0f);
}
...
-(void)constructScatterPlot
{
	...
	plotSpace.xRange				= [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(0.0) length:CPTDecimalFromFloat(16.0)];
	...
	x.orthogonalCoordinateDecimal = CPTDecimalFromString(@"50");
    ...
    newPlotSpace.xRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(0.0) length:CPTDecimalFromFloat(16.0)];
    
    CPTXYAxis *rightY = [[CPTXYAxis alloc] init];
    rightY.coordinate = CPTCoordinateY;
    rightY.orthogonalCoordinateDecimal = CPTDecimalFromString(@"15");
    rightY.majorIntervalLength		  = CPTDecimalFromString(@"50");
	rightY.minorTicksPerInterval		  = 5;
    CPTMutableTextStyle *whiteText = [[CPTMutableTextStyle alloc] init];
    whiteText.color = [CPTColor whiteColor];
    [rightY setAxisTitle:[[CPTAxisTitle alloc] initWithText:@"hello" textStyle:whiteText]];
    rightY.plotSpace = newPlotSpace;
	...
	lineStyle.dashPattern			 = [NSArray arrayWithObjects:[NSNumber numberWithFloat:5.0f], [NSNumber numberWithFloat:2.0f], nil];
	...
	[graph addPlot:dataSourceLinePlot toPlotSpace:newPlotSpace];
...
//	boundLinePlot.plotSymbol = plotSymbol;
	...
	for ( i = 0; i <= 16; i++ ) {
		...
		id y = [NSNumber numberWithFloat:250.0 * rand() / (float)RAND_MAX + 50.0];
...
//					}
                    num = (NSDecimalNumber *)[NSDecimalNumber numberWithFloat:150.0 * rand() / (float)RAND_MAX + 50.0];
				}
				break;
		}
	}
