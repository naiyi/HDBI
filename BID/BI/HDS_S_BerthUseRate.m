...
#import "HDS_S_BerthUseRate.h"

@implementation HDS_S_BerthUseRate{
...
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        case 0: return node.depth == 0?  @"corpNam":@"berthNam";
        case 1: return @"berthLen";
        case 2: return @"dayHour";
        case 3: return @"useTim";
        case 4: return @"workTim";
        case 5: return @"ttlWgt";
        case 6: return @"useRate";
        case 7: return @"workRate";
        case 8: return @"meterWgt";
        default:return @""; 
    }
}
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    NSString *berthNam=[node.properties objectForKey:@"berthNam"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_BerthUseRate_chart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_BerthUseRate_chart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SBerthUseRate/listHbChart.json?beginYM=%@&endYM=%@&corpCod=%@&berthCod=%@",qBeginDate,qEndDate,corpCod,berthCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SBerthUseRate/listTbChart.json?beginYM=%@&endYM=%@&corpCod=%@&berthCod=%@",qBeginDate,qEndDate,corpCod,berthCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
-(double)doubleForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index{
    if ( [plot isKindOfClass:[CPTScatterPlot class]]){
        if(fieldEnum == CPTScatterPlotFieldX){
            ...
            return [(NSNumber *)[dict objectForKey:@"meterWgt"] doubleValue];
        }
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_BerthUseRate_list" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SBerthUseRate/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView1 dataForPlot:dataForPlot1 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"useRate",@"workRate",@"meterWgt",nil] yTitleWidth:isSmallView?16.0f:18.0f plotNum:1];
        
    }else if(parser == parser3){
        [self refreshBarPlotView:plotView2 dataForPlot:dataForPlot2 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"useRate",@"workRate",@"meterWgt",nil] yTitleWidth:isSmallView?16.0f:18.0f plotNum:1];
    } 
}
