...
#import "HDS_C_ShipCorpThruput.h"
...
@implementation HDS_C_ShipCorpThruput{
...
- (void)viewDidLoad
{
    ...
    if(isSmallView){
        ...
        currentViewIndex = 0;
        self.titleLabel.text = @"分船公司吞吐量统计";
    ...
    [self addLinePlotView:plotView1 toContainer:plotContainer1 title:@"船公司吞吐量环比(TEU)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"箱量",nil] useLegend:NO useLegendIcon:NO];
    
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:plotContainer2 title:@"船公司吞吐量同比(TEU)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"箱量",nil] useLegend:NO useLegendIcon:NO];
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    NSString *qCorpCod,*qShipCorpNam;
    ...
    if(node.depth == 0){
        ...
        qShipCorpNam = [node.properties objectForKey:@"name2"];
    }
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipCorpThruputChart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipCorpThruputChart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipCorpThruput/listHbChart.json?level=%i&beginYM=%@&endYM=%@&corpCod=%@&shipCorpNam=%@",qLevel,qBeginDate,qEndDate,qCorpCod,qShipCorpNam] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipCorpThruput/listTbChart.json?level=%i&beginYM=%@&endYM=%@&corpCod=%@&shipCorpNam=%@",qLevel,qBeginDate,qEndDate,qCorpCod,qShipCorpNam] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@吞吐量环比(TEU)",qName];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@吞吐量同比(TEU)",qName];
//    pieView1.hostedGraph.title=[NSString stringWithFormat:@"%@进出口比例",qName.length>6?@"":qName];
//    pieView2.hostedGraph.title=[NSString stringWithFormat:@"%@空重比例",qName.length>6?@"":qName];
//    pieView3.hostedGraph.title=[NSString stringWithFormat:@"%@尺寸比例",qName.length>6?@"":qName];
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipCorpThruputGrid" withExtension:@"json"]]];
    }else{
        NSString *url;
        url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipCorpThruput/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate, qEndDate, qCompany];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"分船公司吞吐量统计");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"分船公司吞吐量统计");
	} else{
