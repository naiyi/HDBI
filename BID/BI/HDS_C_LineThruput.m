...
- (void)viewDidLoad
{
    ...
    [self addLinePlotView:plotView1 toContainer:plotContainer1 title:@"航线吞吐量环比(TEU)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"箱量",nil] useLegend:NO useLegendIcon:NO];
    
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:plotContainer2 title:@"航线吞吐量同比(TEU)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"箱量",nil] useLegend:NO useLegendIcon:NO];
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    NSString *qCorpCod,*qShipLineNam;
    ...
    if(node.depth == 0){
        ...
        qShipLineNam = [node.properties objectForKey:@"name2"];
    }
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_LineThruputChart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_LineThruputChart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CLineThruput/listHbChart.json?level=%i&beginYM=%@&endYM=%@&corpCod=%@&shipLineNam=%@",qLevel,qBeginDate,qEndDate,qCorpCod,qShipLineNam] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CLineThruput/listTbChart.json?level=%i&beginYM=%@&endYM=%@&corpCod=%@&shipLineNam=%@",qLevel,qBeginDate,qEndDate,qCorpCod,qShipLineNam] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_LineThruputGrid" withExtension:@"json"]]];
    }else{
        NSString *url;
        url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CLineThruput/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate, qEndDate, qCompany];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"分航线吞吐量统计");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"分航线吞吐量统计");
	} else{
