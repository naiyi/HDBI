//
//  HDS_S_ClientIncome.m
//  HDBI
//
//  Created by 毅 张 on 12-8-7.
//  Copyright (c) 2012年 烟台华东电子. All rights reserved.
//

#import "HDS_S_ClientIncome.h"

@implementation HDS_S_ClientIncome{
...
- (void)viewDidLoad{
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"计费收入环比变化趋势(万元)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"计费收入同比变化趋势(万元)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    if(isSmallView){
        if(column == 0) return 145;
        if(column == 1) return 76;
        return 85;
    }
    if(column == 0) return 259;
    return 200;
}
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    NSString *clientCod=[node.properties objectForKey:@"clientCod"];
    NSString *client=[node.properties objectForKey:@"client"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ClientIncome_chart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ClientIncome_chart2" withExtension:@"json"]]];
    }else{
        urlHB = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SClientIncome/listHbChart.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@&clientCod=%@",qBeginDate,qEndDate,qCompany,qCargo,clientCod];
        urlTB = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SClientIncome/listTbChart.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@&clientCod=%@",qBeginDate,qEndDate,qCompany,qCargo,clientCod];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@计费收入环比(万元)",client];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@计费收入同比(万元)",client];
}
...
-(double)doubleForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index{
    if ( [plot isKindOfClass:[CPTScatterPlot class]]){
        if(fieldEnum == CPTBarPlotFieldBarLocation ){
            ...
            return [(NSNumber *)[dict objectForKey:@"income"] doubleValue];
        }
    }
    return 0;
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ClientIncome_list" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SClientIncome/list.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@",qBeginDate,qEndDate,qCompany,qCargo];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView1 dataForPlot:dataForPlot1 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"income",nil] yTitleWidth:0 plotNum:1];
        
    }else if(parser == parser3){
        [self refreshBarPlotView:plotView2 dataForPlot:dataForPlot2 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"income",nil] yTitleWidth:0 plotNum:1];
    } 
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"货主贡献度");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"货主贡献度");
	} else{
        //        NSLog(@"Parser over.");
    }
}
