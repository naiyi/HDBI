...
#import "HDS_C_DeviceGoodRate.h"

@implementation HDS_C_DeviceGoodRate{
...
- (void)viewDidLoad{
    ...
    [self addBarPlotView:plotView1 toContainer:barContainer1 title:@"设备完好率比较" xTitle:nil yTitle:nil plotNum:3 identifier:[NSArray arrayWithObjects:@"本月",@"上月",@"去年同期",nil] useLegend:YES useLegendIcon:YES];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"完好率变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceGoodRateChart" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceRate/listHbChart.json?v=1&beginYM=%@&endYM=%@&devCod=%@",qMonth,qMonth,devCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@完好率变化趋势",devNam];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceGoodRateGrid" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceRate/list.json?v=1&yearMonth=%@&corps=%@",qMonth,qCompany];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"设备完好率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"设备完好率");
	} else{
