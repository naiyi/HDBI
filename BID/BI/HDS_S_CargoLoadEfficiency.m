...
#import "HDS_S_CargoLoadEfficiency.h"

@implementation HDS_S_CargoLoadEfficiency{
...
- (void)viewDidLoad{
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"作业效率环比变化趋势(吨/时)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"作业效率环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"作业效率同比变化趋势(吨/时)" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"作业效率同比",nil] useLegend:NO useLegendIcon:NO];
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    NSString *qProcCod=[node.properties objectForKey:@"procCod"];
    NSString *qWayCod=[node.properties objectForKey:@"wayCod"];
    NSString *cargo = [node.properties objectForKey:@"cargoKind"];
    NSString *proc = [node.properties objectForKey:@"proc"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_CargoLoadEfficiency_chart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_CargoLoadEfficiency_chart2" withExtension:@"json"]]];
    }else{
        urlHB = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SCargoLoadRate/listHbChart.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@&procCod=%@&wayCod=%@",qBeginDate,qEndDate,qCompany,qCargoKindCod,qProcCod,qWayCod];
        urlTB = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SCargoLoadRate/listTbChart.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@&procCod=%@&wayCod=%@",qBeginDate,qEndDate,qCompany,qCargoKindCod,qProcCod,qWayCod];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@%@作业效率环比变化趋势(吨/时)",cargo,proc];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@%@作业效率同比变化趋势(吨/时)",cargo,proc];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_CargoLoadEfficiency_list" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SCargoLoadRate/list.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@",qBeginDate,qEndDate,qCompany,qCargo];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"单货种装卸效率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"单货种装卸效率");
	} else{
        //        NSLog(@"Parser over.");
    }
}
