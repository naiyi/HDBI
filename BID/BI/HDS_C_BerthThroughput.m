...
#import "HDS_C_BerthThroughput.h"

@implementation HDS_C_BerthThroughput{
...
- (void)viewDidLoad{
    ...
    if(isSmallView){
        ...
        currentViewIndex = 0;
        self.titleLabel.text = @"泊位通过量分析";
        [self smallViewChangeToIndex:currentViewIndex];
        [self insertPageControlToSmallView];
        [self createConditionView];
    }
    
    headerNames = [NSArray arrayWithObjects:@"公司",@"泊位名称",@"泊位长度",@"总吞吐量(TEU)",@"停泊艘次总数",@"每米码头吞吐量",nil];
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"每米码头吞吐量环比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"每米码头吞吐量同比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        ...
        case 4: return @"shipNum";
        case 5: return @"meterTeu";
        default:return @""; 
    }
}
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_BerthThroughputChart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_BerthThroughputChart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CBerthThroughput/listHbChart.json?beginYM=%@&endYM=%@&corpCod=%@&berthCod=%@",qBeginDate,qEndDate,corpCod,berthCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CBerthThroughput/listTbChart.json?beginYM=%@&endYM=%@&corpCod=%@&berthCod=%@",qBeginDate,qEndDate,corpCod,berthCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@%@每米码头吞吐量环比",corpNam,berthNam];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@%@每米码头吞吐量同比",corpNam,berthNam];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_BerthThroughputGrid" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CBerthThroughput/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView1 dataForPlot:dataForPlot1 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"meterTeu",nil] yTitleWidth:0 plotNum:1];
        
    }else if(parser == parser3){
        [self refreshBarPlotView:plotView2 dataForPlot:dataForPlot2 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"meterTeu",nil] yTitleWidth:0 plotNum:1];
    } 
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"泊位通过率分析");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"泊位通过率分析");
	} else{
