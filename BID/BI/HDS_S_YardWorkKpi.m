...
#import "HDS_S_YardWorkKpi.h"

@implementation HDS_S_YardWorkKpi{
...
- (void)fillConditionView:(UIView *)view{
    ...
    // segment
    segment = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"散货盈亏比例",@"内贸直取比例",@"进出货平衡率", nil]];
    segment.segmentedControlStyle = UISegmentedControlStylePlain;
    segment.frame = CGRectMake(20, 90, 270, 31);
    segment.selectedSegmentIndex = 0;
    [segment addTarget:self action:@selector(changeSegmentTaped:) forControlEvents:UIControlEventValueChanged];
...
- (void)viewDidLoad{
    ...
    if(isSmallView){
        ...
        currentViewIndex = 0;
        self.titleLabel.text = @"库场利用率分析";
        [self smallViewChangeToIndex:currentViewIndex];
        [self insertPageControlToSmallView];
        [self createConditionView];
    }
    
    [HDSUtil changeUIControlFont:segment toSize:isSmallView?HDSFontSizeVerySmall:HDSFontSizeBig];
    
    headerNames = [NSArray arrayWithObjects:@"货类",@"入库吨数",@"出库吨数",@"盈亏比例(％)",@"货类",@"直取吨数",@"自然吨数",@"直取率(％)",@"货类",@"入库吨数",@"出库吨数",@"平衡率(％)",nil];
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"库场利用率环比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"库场利用率同比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
    ...
    [self changeSegmentTaped:segment];
//    [self loadData];
}

- (IBAction)changeSegmentTaped:(UISegmentedControl *)sender {
    if(segment.selectedSegmentIndex == 0){ //月度
...
- (NSString *)tableView:(HDSTableView *)tableView propertyHeaderForColumn:(NSInteger)col{
    return [headerNames objectAtIndex:segment.selectedSegmentIndex*4 + col];
}

// 列宽不包括边框宽度
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    if(isSmallView){
        if(column==0)   return 130.0f;
        if(column==1)   return 90.0f;
        return 90.0f;
    }
    if(column==0)   return 208.0f;
    return 150.0f;
}
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    NSArray *tabNames = [NSArray arrayWithObjects:@"散货盈亏比率",@"内贸直取比率",@"进出库平衡率", nil];
    int index = segment.selectedSegmentIndex;
    NSString *cargoNam=[node.properties objectForKey:@"name"];
    NSString *cargoKindCod=[node.properties objectForKey:@"code"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_YardWorkKpi_chart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_YardWorkKpi_chart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SYardWorkKpi/listHbChart%i.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@",index+1,qBeginDate,qEndDate,qCompany,cargoKindCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SYardWorkKpi/listTbChart%i.json?beginYM=%@&endYM=%@&corps=%@&cargoKindCod=%@",index+1,qBeginDate,qEndDate,qCompany,cargoKindCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@%@环比变化趋势",cargoNam,[tabNames objectAtIndex:index]];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@%@同比变化趋势",cargoNam,[tabNames objectAtIndex:index]];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_YardWorkKpi_list" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SYardWorkKpi/list%i.json?beginYM=%@&endYM=%@&corps=%@",index+1,qBeginDate,qEndDate,qCompany];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView2 dataForPlot:dataForPlot2 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"rate",nil] yTitleWidth:0 plotNum:1];
    } 
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"库场作业效率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"库场作业效率");
	} else{
        //        NSLog(@"Parser over.");
    }
}
