...
#import "HDS_C_DeviceErrorRate.h"

@implementation HDS_C_DeviceErrorRate{
...
- (void)viewDidLoad{
    ...
    [self addBarPlotView:plotView1 toContainer:barContainer1 title:@"设备故障率(％)对比" xTitle:nil yTitle:nil plotNum:3 identifier:[NSArray arrayWithObjects:@"本月",@"上月",@"去年同期",nil] useLegend:YES useLegendIcon:YES];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"故障率变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceErrorRateChart" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceRate/listHbChart.json?v=2&beginYM=%@&endYM=%@&devCod=%@",qMonth,qMonth,devCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@故障率变化趋势",devNam];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceErrorRateGrid" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceRate/list.json?v=2&yearMonth=%@&corps=%@",qMonth,qCompany];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"设备故障率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"设备故障率");
	} else{
