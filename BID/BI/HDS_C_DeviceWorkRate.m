...
#import "HDS_C_DeviceWorkRate.h"

@implementation HDS_C_DeviceWorkRate{
...
- (void)viewDidLoad{
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"设备作业效率环比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"设备作业效率同比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceWorkRateChart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceWorkRateChart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceWorkRate/listHbChart.json?beginYM=%@&endYM=%@&corpCod=%@&devCod=%@",qBeginDate,qEndDate,corpCod,devCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceWorkRate/listTbChart.json?beginYM=%@&endYM=%@&corpCod=%@&devCod=%@",qBeginDate,qEndDate,corpCod,devCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@%@作业效率环比变化趋势(TEU/时)",corpNam,devNam];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@%@作业效率同比变化趋势(TEU/时)",corpNam,devNam];
}
...
-(CPTLayer *)dataLabelForPlot:(CPTPlot *)plot recordIndex:(NSUInteger)index{
    ...
    if(num > 0){
        newLayer = [[CPTTextLayer alloc] initWithText:[NSString stringWithFormat:@"%g", num] style:[HDSUtil plotTextStyle:10]];
    }
	return newLayer;
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_DeviceWorkRateGrid" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CDeviceWorkRate/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"主要设备作业效率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"主要设备作业效率");
	} else{
