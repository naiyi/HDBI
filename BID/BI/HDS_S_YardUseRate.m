...
#import "HDS_S_YardUseRate.h"

@implementation HDS_S_YardUseRate{
...
- (void)viewDidLoad{
    ...
    headerNames = [NSArray arrayWithObjects:@"库场名称",@"堆存合计",@"期初结存",@"本期进库",@"本期出库",@"期末结存",@"货物堆存吨天",@"平均堆存期",@"平均每天堆存吨数",@"容量周转次数",@"库场利用率",nil];
...
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    if(isSmallView){
        ...
        if(column <= 5) return 65;
        if(column == 6) return 90;
        if(column == 7) return 70;
        if(column == 8) return 115;
        return 90;
    }
    if(column == 0) return 70;
    if(column <= 5) return 85;
    if(column == 6) return 105;
    if(column == 7) return 90;
    if(column == 8) return 130;
    return 100;
}

// @"库场名称",@"堆存合计",@"期初结存",@"本期进库",@"本期出库",@"货物堆存吨天",@"平均堆存期",@"平均每天堆存吨数",@"容量周转次数",@"库场利用率"
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        case 0: return @"yardNam";
        case 1: return @"totalWgt";
        case 2: return @"begWgt";
        case 3: return @"totalInWgt";
        case 4: return @"totalOutWgt";
        case 5: return @"endWgt";
        case 6: return @"stockWgtDay";
        case 7: return @"avgDay";
        case 8: return @"avgDayWgt";
        case 9: return @"turnWgt";
        case 10:return @"useRate";
        default:return @""; 
    }
}

-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    NSString *yardNam=[node.properties objectForKey:@"yardNam"];
    NSString *yardCod=[node.properties objectForKey:@"yardCod"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_YardUseRate_chart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_YardUseRate_chart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SYardUseRate/listHbChart.json?beginYM=%@&endYM=%@&corps=%@&YardCod=%@",qBeginDate,qEndDate,qCompany,yardCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SYardUseRate/listTbChart.json?beginYM=%@&endYM=%@&corps=%@&YardCod=%@",qBeginDate,qEndDate,qCompany,yardCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@库场利用率环比变化趋势",yardNam];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@库场利用率同比变化趋势",yardNam];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_YardUseRate_list" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SYardUseRate/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView2 dataForPlot:dataForPlot2 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"useRate",nil] yTitleWidth:0 plotNum:1];
    } 
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"库场利用率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"库场利用率");
	} else{
        //        NSLog(@"Parser over.");
    }
}
