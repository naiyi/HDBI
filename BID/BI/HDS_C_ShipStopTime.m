...
#import "HDS_C_ShipStopTime.h"

@implementation HDS_C_ShipStopTime{
...
- (void)viewDidLoad{
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"船舶停时环比分析" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"船舶停时同比分析" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    ...
    if(column <= 3) return 135.0f;
    return 80.0f;
}

// @"船名航次",@"泊位",@"靠泊时间",@"离港时间",@"作业量",@"生产停时",@"非生产停时"
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        case 0: return @"ship";
        case 1: return @"berth";
        case 2: return @"arrvTim";
        case 3: return @"leavTim";
        case 4: return @"teu";
        case 5: return @"prodStopHours";
        case 6: return @"nonProdStopHours";
        default:return @""; 
    }
}
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    ...
    NSString *shipNam=[node.properties objectForKey:@"ship"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipStopTimeChart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipStopTimeChart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipStopTime/listHbChart.json?beginYM=%@&endYM=%@&shipCod=%@&corpCod=%@",qBeginDate,qEndDate,shipCod,corpCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipStopTime/listTbChart.json?beginYM=%@&endYM=%@&shipCod=%@&corpCod=%@",qBeginDate,qEndDate,shipCod,corpCod] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@停时环比分析",shipNam];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@停时同比分析",shipNam];
}
...
-(double)doubleForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index{
    if ( [plot isKindOfClass:[CPTScatterPlot class]]){
        if(fieldEnum == CPTScatterPlotFieldX){
            ...
            return [(NSNumber *)[dict objectForKey:@"hours"] doubleValue];
        }
    }
    return 0;
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipStopTime" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipStopTime/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
...
//            [self loadChildren:node withData:data];
        ...
        [self refreshBarPlotView:plotView1 dataForPlot:dataForPlot1 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"hours",nil] yTitleWidth:0 plotNum:1];
        
    }else if(parser == parser3){
        [self refreshBarPlotView:plotView2 dataForPlot:dataForPlot2 data:array dataIsTreeNode:false  xLabelKey:@"date" yLabelKey:[NSArray arrayWithObjects:@"hours",nil] yTitleWidth:0 plotNum:1];
    } 
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"船舶停时分析");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"船舶停时分析");
	} else{
