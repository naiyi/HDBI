...
#import "HDS_S_IOPortPlanComplete.h"

@implementation HDS_S_IOPortPlanComplete{
...
- (void)fillConditionView:(UIView *)view{
    // segment集港/疏港
    segment = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"集港",@"疏港", nil]];
    ...
    dateBtn.frame = CGRectMake(131,10,119,31);
...
- (void)viewDidLoad{
    ...
    if(isSmallView){
        ...
        currentViewIndex = 0;
        self.titleLabel.text = @"集疏港计划完成情况";
    ...
    headerNames = [NSArray arrayWithObjects:@"船名",@"货主",@"货名",@"计划吨数",@"完成吨数",
                   @"完成比例(％)", nil];
    ...
    [self addBarPlotView:plotView1 toContainer:plotContainer1 title:@"船舶集疏港完成情况对比" xTitle:nil yTitle:nil plotNum:2 identifier:[NSArray arrayWithObjects:@"完成",@"计划",nil] useLegend:YES useLegendIcon:NO];
...
- (IBAction)segmentChanged:(UISegmentedControl *)sender {
    if([sender selectedSegmentIndex] == 0){ //集港
        plotView1.hostedGraph.title = @"各船舶集港完成情况对比";
        qIoPort = @"JG";
    }else{  //疏港
        plotView1.hostedGraph.title = @"各船舶疏港完成情况对比";
        qIoPort = @"SG";
    }
    [self loadData];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_IOPortPlanComplete_list" withExtension:@"json"]]];
    }else{
        NSString *url;
        url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SIoPortPlan/list.json?type=%@&date=%@&corps=%@",qIoPort,qDate,qCompany];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView1 dataForPlot:dataForPlot1 data:self.rootArray dataIsTreeNode:true xLabelKey:@"ship" yLabelKey:[NSArray arrayWithObjects:@"plan",@"work",nil] yTitleWidth:0 plotNum:2];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"集疏港计划完成");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"集疏港计划完成");
	} else{
        //        NSLog(@"Parser over.");
    }
}
