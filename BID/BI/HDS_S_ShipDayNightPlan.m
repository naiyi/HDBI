...
#import "HDS_S_ShipDayNightPlan.h"

@implementation HDS_S_ShipDayNightPlan{
...
- (void)viewDidLoad{
    ...
    [self addBarPlotView:plotView1 toContainer:plotContainer1 title:@"船舶作业计划完成情况对比" xTitle:nil yTitle:nil plotNum:2 identifier:[NSArray arrayWithObjects:@"完成",@"计划",nil] useLegend:YES useLegendIcon:NO];
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_ShipDayNightPlan_list" withExtension:@"json"]]];
    }else{
        NSString *url;
        url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SShipDayWorkPlan/list.json?date=%@&corps=%@",qDate,qCompany];
