...
#import "HDS_S_DeviceUseRate.h"

@implementation HDS_S_DeviceUseRate{
...
- (void)viewDidLoad{
    ...
    [self addLinePlotView:plotView1 toContainer:barContainer1 title:@"利用率环比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"环比",nil] useLegend:NO useLegendIcon:NO];
    plotView2 = [[CPTGraphHostingView alloc] initWithFrame:CGRectZero];
    [self addLinePlotView:plotView2 toContainer:barContainer2 title:@"利用率同比变化趋势" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"同比",nil] useLegend:NO useLegendIcon:NO];
...
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    if(isSmallView){
        if(column == 0) return 105;
        return 60;
    }
    if(column == 0) return 131;
    return 105;
}

// @"设备类别",@"日历台时",@"完好台时",@"运行台时",@"完好率",@"利用率"
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        case 0: return @"name";
        case 1: return @"calendarHour";
        case 2: return @"goodHour";
        case 3: return @"workHour";
        case 4: return @"goodRate";
        case 5: return @"workRate";
        default:return @""; 
    }
}
...
-(void)tableView:(HDSTableView *)tableView didSelectRowAtNode:(HDSTreeNode *)node{
    NSString *name=[node.properties objectForKey:@"name"];
    NSString *code=[node.properties objectForKey:@"code"];
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_DeviceUseRate_chart1" withExtension:@"json"]]];
        [parser3 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_DeviceUseRate_chart2" withExtension:@"json"]]];
    }else{
        urlHB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SDeviceUseRate/listHbChart.json?level=%i&beginYM=%@&endYM=%@&corps=%@&code=%@",level,qBeginDate,qEndDate,qCompany,code] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlTB = [[[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SDeviceUseRate/listTbChart.json?level=%i&beginYM=%@&endYM=%@&corps=%@&code=%@",level,qBeginDate,qEndDate,qCompany,code] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    plotView1.hostedGraph.title = [NSString stringWithFormat:@"%@利用率环比变化趋势",name];
    plotView2.hostedGraph.title = [NSString stringWithFormat:@"%@利用率同比变化趋势",name];
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_S_DeviceUseRate_list" withExtension:@"json"]]];
    }else{
        // 表格数据
        NSString *url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/SDeviceUseRate/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
