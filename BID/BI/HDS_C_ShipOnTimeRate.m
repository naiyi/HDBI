...
#import "HDS_C_ShipOnTimeRate.h"

@implementation HDS_C_ShipOnTimeRate{
...
- (void)viewDidLoad{
    ...
    [self addBarPlotView:plotView1 toContainer:plotContainer1 title:@"航线班轮准班率" xTitle:nil yTitle:nil plotNum:1 identifier:[NSArray arrayWithObjects:@"航线",nil] useLegend:NO useLegendIcon:NO];
...
- (CGFloat)tableView:(HDSTableView *)tableView widthForColumn:(NSInteger)column{
    if(isSmallView){
        ...
        if(column <= 4) return 100.0f;
        return 60.0f;
    }
    if(column == 0) return 116.0f;
    if(column <= 2) return 110.0f;
    if(column <= 4) return 120.0f;
    return 80.0f;
}
...
- (NSString *)tableView:(HDSTableView *)tableView propertyNameForColumn:(NSInteger)col node:(HDSTreeNode *)node{
    switch (col) {
        case 0: return @"corp";
        case 1: return @"lineKind";
        case 2: return @"line";
        case 3: return @"linerNum";
        case 4: return @"realNum";
        case 5: return @"realRate";
        default:return @"";
    }
}
...
- (double)doubleForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index{
    if(fieldEnum == CPTBarPlotFieldBarLocation ){
        ...
        return [(NSNumber *)[dict objectForKey:@"realRate"] doubleValue];
    }
}
...
- (void)loadData {
    ...
    if([HDSUtil isOffline]){    // 离线数据
        [parser1 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipOnTimeRateGrid" withExtension:@"json"]]];
        [parser2 parse:[[NSData alloc] initWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"HDS_C_ShipOnTimeRateChart" withExtension:@"json"]]];
    }else{
        NSString *url,*url2;
        url = [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipOnTimeRate/list.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
        url2= [[HDSUtil serverURL] stringByAppendingFormat: @"/bi_pad/CShipOnTimeRate/listChart.json?beginYM=%@&endYM=%@&corps=%@",qBeginDate,qEndDate,qCompany];
...
- (void)parser:(SBJsonStreamParser *)parser foundArray:(NSArray *)array {
    ...
    if(parser == parser1){
        ...
        [self refreshBarPlotView:plotView1 dataForPlot:dataForPlot1 data:array dataIsTreeNode:false  xLabelKey:@"line" yLabelKey:[NSArray arrayWithObjects:@"realRate",nil] yTitleWidth:0 plotNum:1];
    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
	...
	if (status == SBJsonStreamParserError) {
		NSLog(@"The parser encountered an error: %@ in %@.", parser.error,@"航线班轮准班率");
	} else if (status == SBJsonStreamParserWaitingForData) {
		NSLog(@"Parser waiting for more data in %@",@"航线班轮准班率");
	} else{
