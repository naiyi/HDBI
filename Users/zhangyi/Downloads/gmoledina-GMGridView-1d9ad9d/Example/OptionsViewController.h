//
//  OptionsViewController.h
//  GMGridView
//
//  Created by Gulam Moledina on 11-11-01.
//  Copyright (c) 2011 GMoledina.ca. All rights reserved.
//

#import <UIKit/UIKit.h>

@class GMGridView;
@class HDSMasterViewController;

@interface OptionsViewController : UIViewController

@property (nonatomic, strong) GMGridView *gridView;
@property (unsafe_unretained, nonatomic) UIPopoverController *popoverController;
@property (nonatomic, strong) HDSMasterViewController *masterVC;

+ (OptionsViewController *) sharedOptionViewController;
+ (UIPopoverController *) sharedOptionPopoverController;

@end
